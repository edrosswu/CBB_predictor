...
headers3 = [str(i) for i in range(263)]
...
#change path 
headers4 = ['Seed', 'Team']
south = pd.read_csv(r"C:\Users\Adam\Downloads\south_region.csv")
south = pd.DataFrame(south, columns=headers4)
east = pd.read_csv(r"C:\Users\Adam\Downloads\east_region.csv")
east = pd.DataFrame(east, columns=headers4)
west = pd.read_csv(r"C:\Users\Adam\Downloads\west_region.csv")
west = pd.DataFrame(west, columns=headers4)
midwest = pd.read_csv(r"C:\Users\Adam\Downloads\midwest_region.csv")
midwest = pd.DataFrame(midwest, columns=headers4)
full = pd.read_excel(r"C:\Users\Adam\Downloads\bracket.xlsx")
...
def matchup(team1, team2):    # defensive efficiencies (higher values are worse)
    ...
    seed_factor1 = math.exp(-0.175 * (seed1 - 1))  # A gentle decay for higher seeds
    seed_factor2 = math.exp(-0.175 * (seed2 - 1))  # Same adjustment for team 2's seed

    # Adjusted win probabilities considering turnovers, shooting efficiency, and defense rebound rate
    adj_win_pct1 = win_pct1 * (1 - to_rate1) * efg1 * (1 - drb1) * seed_factor1
    adj_win_pct2 = win_pct2 * (1 - to_rate2) * efg2 * (1 - drb2) * seed_factor2
    # Calculate the total expected winning percentage of the game
    total_win_pct = adj_win_pct2 / (adj_win_pct1 + (adj_win_pct2))
    team2_prob = total_win_pct
    team1_prob = 1- total_win_pct
    
    winner = team1 if random.uniform(0, 1) < team1_prob else team2
    winner_prob = team1_prob if winner == team1 else team2_prob
    winner_seed = seed1 if winner == team1 else seed2
    return winner, winner_prob, winner_seed
...
def simulate_regions():
    ...
    final_four_df = pd.concat([south_winner, west_winner , east_winner, midwest_winner], ignore_index=True)
